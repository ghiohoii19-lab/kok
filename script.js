...
canvas.addEventListener("mousedown", e => {
  ...
  isMovingSelection.forEach((corner, i) => {
    const dx = x - corner.x;
    const dy = y - corner.y;
    if (Math.hypot(dx, dy) < 20) {
      draggingCorner = i;
...
const toggleToolsBtn = document.getElementById('toggle-panels-btn');
const toggleLayersBtn = document.getElementById('toggle1');

// toggle tools panel
toggleToolsBtn.addEventListener('click', () => {
  toolsPanel.classList.toggle('show');
  toggleToolsBtn.classList.toggle('shift', toolsPanel.classList.contains('show'));
});

// toggle layers panel
toggleLayersBtn.addEventListener('click', () => {
  layersPanel.classList.toggle('show');
  toggleLayersBtn.classList.toggle('shift', layersPanel.classList.contains('show'));
});
...
switch (brushStyle) {
   ...
   case 'watercolor':
    ...
    for (let i = 0; i < 70; i++) { // Jumlah partikel 
        const angle = Math.random() * Math.PI * 2;
    ...
    ctx.globalAlpha = 0.02 * pigmentFlow; 
    ctx.globalCompositeOperation = 'multiply';
  ...
  if (count > 0) {
    ...
    const blendedColor = `rgba(${r},${g},${b},0.1)`; 
...
layersList.addEventListener("pointermove", e => {
  ...
  const { y } = getEventXY(e); // pakai utilitas kamu
...
canvasContainer.addEventListener('touchmove', e => {
  if (e.touches.length === 1 && isDrawing) {
    draw(e.touches[0]); 
    render();
  } else if (e.touches.length === 2 && isGesture && initialTouches) {
    e.preventDefault();
    const [t1, t2] = e.touches;
    const dx = t1.clientX - t2.clientX;
    const dy = t1.clientY - t2.clientY;
